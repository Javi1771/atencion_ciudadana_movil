// lib/data/menu_options.dart

///? Opciones para menús desplegables
class MenuOptions {
  static const List<String> colonias = [
    'Alejandrina',
    'Ampliación la Llave',
    'Ampliación la Loma',
    'Ampliación Loma Linda',
    'Ampliación Vistha Norte',
    'Ana',
    'Aquiles Serdán',
    'Aquiles Serdán Sección Libertad',
    'Arboledas',
    'Arcila',
    'Arcos del Sol',
    'Arrayanes',
    'Banthí',
    'Barranca de Cocheros',
    'Benito Juárez',
    'Berber',
    'Betania',
    'Bosques de Banthi',
    'Bosques de San Juan',
    'Buenavista',
    'Buenavista (Lindavista)',
    'Buenavista Palma de Romero',
    'Buenos Aires',
    'Bugambilias',
    'Campestre San Juan',
    'Casa Blanca',
    'Centro',
    'Centro de Readaptación Social',
    'Centro Urbano Nuevo San Juan',
    'Cerro Gordo',
    'CFE',
    'Club de Golf San Juan',
    'Club Residencial San Isidro',
    'Cofradía Grande',
    'Colibrí del Pedregal',
    'Colinas de Oriente',
    'Comevi Banthi',
    'Conca',
    'Conjunto Bugambilias',
    'Country',
    'Cristo Rey',
    'Cuarto Centenario',
    'Cuauhtémoc',
    'De Banthí',
    'De Enmedio',
    'Del Valle',
    'Deportiva',
    'Desarrollo Habitacoinal Los Cipreces',
    'Dolores Cuadrilla de Enmedio',
    'Dolores Godoy',
    'Ejido Cerro Gordo',
    'El Bosque',
    'El Campanario',
    'El Canelo',
    'El Capricho',
    'El Carmen',
    'El Carrizo',
    'El Cazadero',
    'El Chaparro',
    'El Colibri de Banthi',
    'El Coto',
    'El Dorado',
    'El Espárrago',
    'El Granjeno',
    'El Jazmín',
    'El Marfil',
    'El Mirador',
    'El Molino',
    'El Organal',
    'El Paraíso',
    'El Pedregal',
    'El Porvenir',
    'El Riel',
    'El Rincón de Santa Rita',
    'El Rocio',
    'El Rodeo',
    'El Rodeo 2a Sección',
    'El Rosario',
    'El Sitio',
    'Espíritu Santo',
    'Espíritu Santo (Familia Verver)',
    'Estancia de Bordos',
    'Estancia de Santa Lucía',
    'Fátima',
    'Ferrocarrileros',
    'Fluminense',
    'Francisco Villa',
    'Granjas Banthi',
    'Granjas Banthi 3ra Sección',
    'Granjas Banthí Sección Solares',
    'Guadalupe',
    'Guadalupe de las Peñas',
    'Hacienda Las Nueces',
    'Haciendas San Juan',
    'Heraclio Bernal',
    'Huerta Grande',
    'Huizache',
    'INDECO',
    'Independencia',
    'INFONAVIT Fatima',
    'INFONAVIT la Paz',
    'INFONAVIT los Fresnos',
    'INFONAVIT Pedregoso',
    'INFONAVIT Pedregoso 3a Sección',
    'INFONAVIT San Cayetano',
    'INFONAVIT San Isidro',
    'Insurgentes',
    'Jacarandas',
    'Jardines Banthi',
    'Jardines del Pedregal',
    'Jardines del Valle',
    'Jardines del Valle II',
    'Jardines del Valle III',
    'Jardines de San Juan',
    'Josefa Ortíz (4a. Sección del Rodeo)',
    'José López Portillo',
    'Juárez',
    'La Arboleda',
    'La Caseta',
    'La Concepción',
    'La Corregidora',
    'La Cruz',
    'La Esmeralda',
    'La Esperanza',
    'La Estancia',
    'La Estancita',
    'La Floresta',
    'La Guadalupana',
    'La Guitarrilla',
    'Laguna de Lourdes',
    'Laguna de Vaquerías',
    'La Herradura',
    'La Laborcilla',
    'La Ladera',
    'La Ladrillera (Ejido la Llave)',
    'La Llave',
    'La Loma (Ampliación Santa Cruz)',
    'La Magdalena',
    'La Mesa (La Mesa de San Antonio)',
    'La Mina',
    'La Misión',
    'La Mora',
    'La Paz',
    'La Peña 1a. Sección',
    'La Peña 2a. Sección',
    'La Peña 3a. Sección',
    'La Rueda',
    'Las Águilas',
    'Las Águilas I',
    'Las Águilas II',
    'Las Águilas III',
    'Las Alamedas',
    'Las Fuentes',
    'Las Haciendas',
    'Las Huertas',
    'Las Margaritas',
    'Las Palomas',
    'Las Rosas',
    'Las Torres',
    'La Valla',
    'La Venta',
    'La Viña',
    'Lázaro Cárdenas (1a. Secc. del Rodeo)',
    'Loma Alta',
    'Loma de la Valla (Buenavista de la Valla)',
    'Loma Linda',
    'Lomas de Guadalupe',
    'Lomas de la Estancia',
    'Lomas del Pedregal',
    'Lomas de San Juan',
    'Lomas de San Juan 2da. Sección',
    'Lomas de San Juan Sección Campestre',
    'Lomas de San Juan Sección Jardines',
    'Lomo de Toro',
    'Los Cerritos San Miguel',
    'Los Compadres',
    'Los Frailes',
    'Los Fresnos',
    'Los Girasoles',
    'Los Llanitos',
    'Los Nogales',
    'Los Olivos 1a. Sección',
    'Los Olivos 2a. Sección',
    'Los Ordaz',
    'Los Prados',
    'Magisterial',
    'Manantiales de San Juan',
    'México',
    'Milagrito',
    'Militar',
    'Misión de Santa Cruz',
    'Neria de Dolores Godoy',
    'Norte Nuevo Espíritu Santo',
    'Nueva Esperanza',
    'Nuevo Banthí (Ampliación Banthí)',
    'Nuevo Espíritu Santo',
    'Nuevo Parque Industrial',
    'Nuevo San Germán',
    'Nuevo San Isidro',
    'Nuevo San Juan',
    'Ojo de Agua',
    'Ojo de Agua de Vaquerías',
    'Palma de Romero',
    'Palmillas',
    'Parque Industrial Nuevo San Juan',
    'Paseo de las Granadas',
    'Paseo de las Palmas',
    'Paseos del Campestre',
    'Paseos de San Isidro',
    'Paseos de Xhosda',
    'Paso de Mata',
    'Paso de Mata zona IV',
    'Paso de Mata zona VII',
    'Pedregal de San Juan',
    'Perales',
    'Praderas del Sol',
    'Praderas de Oriente',
    'Prados de Cerro Gordo',
    'Prados de Oriente',
    'Prados de San Juan Bautista',
    'Privada del Parque Condominio "B"',
    'Pueblo Nuevo',
    'Pueblo Quieto',
    'Puerta de Alegrías (Puerto de Alegrías)',
    'Puerta de Palmillas',
    'Quintas de Guadalupe',
    'Quintas Esmeralda',
    'Ramos Millán',
    'Rancho Banthí (Familia Navarrete)',
    'Rancho Cuatro Vientos',
    'Real de los Nogales',
    'Residencial Ahuehuetes',
    'Residencial Claustros del Río',
    'Residencial del Ángel',
    'Residencial la Isla',
    'Residencial Las Flores',
    'Residencial Los Cedros',
    'Revolución',
    'Rinconada Banthí',
    'Rinconada del Río',
    'Rinconada de San Juan',
    'Rinconada los Nogales',
    'Rinconada Santa Cruz Nieto',
    'Rincón de los Azhares',
    'Rosa de Castilla',
    'Sabino Chico',
    'Sagrado Corazón',
    'Salto de Vaquerías',
    'San Antonio la Labor',
    'San Antonio Zatlauco',
    'San Cayetano',
    'San Francisco',
    'San Germán',
    'San Gil',
    'San Isidro',
    'San Isidro Labrador',
    'San Javier',
    'San José',
    'San José Galindo',
    'San Juan Bosco',
    'San Juan de Dios',
    'San Martín',
    'San Miguel Arcángel',
    'San Miguel Galindo',
    'San Pablo Potrerillos',
    'San Pedro 1a Sección',
    'San Pedro 2a. Sección',
    'San Pedro 3a Sección',
    'San Pedro Ahuacatlán',
    'San Rafael',
    'San Sebastián de las Barrancas',
    'San Sebastián de las Barrancas Norte',
    'San Sebastián de las Barrancas Sur',
    'Santa Anita',
    'Santa Bárbara de la Cueva',
    'Santa Cruz Escandón',
    'Santa Cruz Nieto',
    'Santa Elena',
    'Santa Fe',
    'Santa Isabel',
    'Santa Isabel el Coto',
    'Santa Lucia',
    'Santa Matilde',
    'Santa Rita',
    'Santa Rosa Xajay',
    'Senegal de las Palomas',
    'Siglo XXI',
    'Soledad del Río',
    'Tercera Sección del Rodeo',
    'Tomasita',
    'Tuna Mansa',
    'Valle de Oro',
    'Valle Dorado 2',
    'Vaquerías',
    'Vegas del Río',
    'Villa de las Haciendas',
    'Villa las Flores',
    'Villa los Cipreses',
    'Villa los Olivos',
    'Villas Corregidora',
    'Villas del Centro',
    'Villas del Parque',
    'Villas del Pedregal',
    'Villas del Pedregal 1a. Sección',
    'Villas del Puente',
    'Villas del Sol',
    'Villas de San Isidro',
    'Villas de San José',
    'Villas de San Juan',
    'Villas Fundadores',
    'Villas Jardín',
    'Villas la Herradura',
    'Villas las Fuentes',
    'Villas los Pirules',
    'Villas Tabachines',
    'Vista Hermosa',
    'Vistha',
    'Viveros de San Juan',
  ];

  static const List<String> tiposSolicitante = [
    'Asociaciones',
    'Ciudadano',
    'Delegado/Subdelegado',
    'Entidad Estatal',
    'Entidad Federal',
    'Entidad Municipal',
    'Escuela',
    'Presidente de Colonos',
    'Proveedor',
    'Regidor',
  ];

  static const List<String> origenes = [
    'Jornadas',
    'Correo electrónico',
    'Giras',
    'Llamada Telefónica',
    'Visita Personal',
  ];

  static const List<String> motivos = [
    'Agradecimiento',
    'Citas',
    'Informes',
    'Invitación',
    'Queja',
    'Solicitud',
    'Trámite',
  ];

  static const List<String> secretarias = [
    'DESARROLLO INTEGRAL DE LA FAMILIA',
    'JEFATURA DE GABINETE',
    'JEFATURA DE LA OFICINA DE LA PRESIDENCIA MUNICIPAL',
    'JUNTA DE AGUA POTABLE Y ALCANTARILLADO MUNICIPAL',
    'SECRETARIA DE ADMINISTRACION',
    'SECRETARIA DE CENTRO DE ATENCION MUNICIPAL',
    'SECRETARIA DE DESARROLLO AGROPECUARIO',
    'SECRETARIA DE DESARROLLO INTEGRAL Y ECONOMICO',
    'SECRETARIA DE DESARROLLO SOCIAL',
    'SECRETARIA DE FINANZAS',
    'SECRETARIA DE GOBIERNO',
    'SECRETARIA DE LA MUJER',
    'SECRETARIA DE OBRAS PUBLICAS Y DESARROLLO URBANO',
    'SECRETARIA DE SEGURIDAD PUBLICA',
    'SECRETARIA DE SERVICIOS PUBLICOS MUNICIPALES',
    'SECRETARIA DEL AYUNTAMIENTO',
    'SECRETARIA DEL ORGANO INTERNO DE CONTROL',
    'SECRETARIA PARTICULAR',
  ];
}